...
def validate_tax_data_new(
    tax_data, relevant_tax_years, tax_years_present, tax_year_to_file
):
    ...
    try:
        ...
        for tax in tax_data:
            ...
            if tax_value.get("calendar_year") and tax_value["calendar_year"].isdigit():
                ...
                # Determine relevancy
                relevancy_status = (
                    "Relevant" if tax_year in relevant_tax_years else "Non-Relevant"
                )

                # Create structured tax file data
                tax_file_data = {
                    "tax_file": tax_key,
                    "doc_id": tax_value.get(
                        "doc_id"
                    ),  # File ID directly from the tax data
    ...
    try:
        for address, data in merged_data.items():
            ...
            for rent_roll in rent_rolls:
                rent_roll_date = datetime.strptime(
                    rent_roll["data"]["date"], "%Y-%m-%d"
                )
                ...
                for lease in leases:
                    lease_start_date = datetime.strptime(
                        lease["data"]["start_date"], "%Y-%m-%d"
                    )
                    lease_end_date = datetime.strptime(
                        lease["data"]["end_date"], "%Y-%m-%d"
                    )
                    ...
                    relevancy_status = (
                        "Relevant"
                        if lease_start_date <= rent_roll_date <= lease_end_date
                        else "Non-Relevant"
                    )
    ...
    try:
        for address, data in merged_data.items():
            ...
            if not rent_rolls:
                ...
                time_period_validator_report[address] = {
                    "issues": issues,
                    "details": details,
                }
                continue
...
def validate_rent_to_tax_old(rent_roll_data, tax_data):
    ...
    try:
        ...
        for rent in rent_roll_data:
            ...
            if not rent_date:
                raise ValueError(
                    f"Missing 'date' in rent roll data for rent_key: {rent_key}"
                )
...
def validate_rent_to_tax_new(rent_roll_data, tax_data):
    ...
    time_period_validator_report = (
        []
    )  # To store the full report, including details and issues
    ...
    try:
        ...
        for rent in rent_roll_data:
            ...
            for tax in tax_data:
                ...
                # Determine relevancy
                relevancy_status = (
                    "Relevant" if rent_year == tax_year else "Non-Relevant"
                )
                ...
                if relevancy_status == "Non-Relevant":
                    ...
                    issues.append(
                        report_entry
                    )  # Add to issues for non-relevant tax files
                else:
                    details.append(
                        report_entry
                    )  # Add to details for relevant tax files

            # Append the result for this rent roll to the time_period_validator_report
            time_period_validator_report.append(
                {"rent_key": rent_key, "details": details, "issues": issues}
            )
        ...
        for rent_key, rent_value in item.items():
            ...
            if file_id:
                mapped_rent_data.append(
                    {"rent_key": rent_key, "data": rent_value, "doc_id": file_id}
                )
            else:
                mapped_rent_data.append(
                    {"rent_key": rent_key, "data": rent_value, "doc_id": None}
                )
